...
5. Validate VPC Keep-Alive send interface status.
   [Documentation]  Validate VPC Keep-Alive send interface status.
    run "show vpc peer-keepalive"
    ${status}=  Run Keyword And Return Status  should be true  values "Sent on interface" and "${VPC_KEEPALIVE_SEND_INTF}" exist on same line
...
6. Validate VPC Keep-Alive Destination Address.
   [Documentation]  Validate VPC Keep-Alive Destination Address.
    run "show vpc peer-keepalive"
    ${status}=  Run Keyword And Return Status  should be true  values "Destination" and "${VPC_KEEPALIVE_DEST_IP}" exist on same line
...
7. Validate VPC Peer-Link status.
   [Documentation]  Validate VPC Peer-Link status.
    run "show vpc brief"
    ${status}=  Run Keyword And Return Status  should be true  values "Peer status" and "peer adjacency formed ok" exist on same line
